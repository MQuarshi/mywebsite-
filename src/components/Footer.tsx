      ...
      <div className="container mx-auto px-4 py-12 sm:px-6 lg:px-8">
        <nav className="flex flex-wrap justify-center -mx-5 -my-2" aria-label="Footer">
          {navigation.main.map((item) => (
            <div key={item.name} className="px-5 py-2">
              <Link to={item.href} className="text-base text-gray-500 hover:text-primary-pink">
                {item.name}
              </Link>
            </div>
          ))}
        ...
        <div className="mt-8 flex justify-center space-x-6">
          {navigation.social.map((item) => (
            <a key={item.name} href={item.href} className="text-gray-400 hover:text-primary-pink">
        ...
        <div className="mt-8 flex flex-col items-center space-y-4">
          ...
          <p className="text-center text-base text-gray-400">
            &copy; {new Date().getFullYear()} In The Moment Photo Booth. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
